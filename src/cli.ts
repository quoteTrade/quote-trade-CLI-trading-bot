...
const engine = new TriggerEngine(triggers, positions, executor, {
  onTrigger: (t, o) => console.log(`${GREEN}✅ Trigger fired${RESET} ${t.id}: submitted ${o.type} ${o.side} ${o.symbol} qty=${o.quantity}${o.price ? ` limit=${o.price}` : ""}`),
...
function parseRiskMetric(value: string): any {
  const metric = String(value ?? "").trim().toUpperCase().replace(/-/g, "_");
...
function parseRiskAction(value: string): any {
  const action = String(value ?? "").trim().toUpperCase().replace(/-/g, "_");
...
function exitSizingFromOptions(opts: any): Pick<TriggerInput, "quantity" | "closePosition" | "closePercentage"> {
  ...
  return sizing.quantity === undefined && !sizing.closePosition && sizing.closePercentage === undefined ? { closePosition: true } : sizing;
}
...
function runTriggerWatcher(symbols?: string[]): void {
  ...
  const watchedSymbols = symbols?.length
    ? symbols.map(normalizeSymbol)
    : triggers.watchableSymbols();
  ...
  for (const symbol of watchedSymbols) {
    ...
    stops.push(feed.start(symbol, 1000, (tick) => {
        if (String(process.env.CLI_DEBUG ?? "").toLowerCase() === "1") {
            console.log("[TRIGGER_DEBUG] RAW_TICK", JSON.stringify({
                symbol,
                price: tick.price,
                bid: tick.bid,
                ask: tick.ask,
  ...
  console.log(`${GREEN}Watching triggers${RESET}${watchedSymbols.length ? ` for: ${watchedSymbols.join(", ")}` : ""}. Stop with Ctrl+C.`);
  const shutdown = () => { stops.forEach((s) => s()); clearInterval(timer); bus.stop(); process.exit(0); };
...
program.command("help").description("Show usage").action(() => console.log(HelpText));
...
program.command("rsi:disable").requiredOption("--symbol <symbol>").action((opts) => {
  const k = normalizeSymbol(opts.symbol);
  const r = runners.get(k);
  if (r) r.stop();
  runners.delete(k);
});
program.command("rsi:status").option("--symbol <symbol>").action((opts) => {
  if (opts.symbol) {
    const r = runners.get(normalizeSymbol(opts.symbol));
    console.log(r ? r.status() : `${BLUE}ℹ️ No runner${RESET}`);
  } else console.log([...runners.entries()].map(([k, r]) => `${k}: ${r.status()}`).join("\n") || `${BLUE}ℹ️ No active runners${RESET}`);
});
...
commonOrderOptions(program.command("trigger:limit")
  .requiredOption("--symbol <symbol>")
  ...
  .action(async (opts) => {
    await resolveForCommand(opts);
    const symbol = normalizeSymbol(opts.symbol);
    printCreated(triggers.add({ kind: "LIMIT", symbol, side: normalizeSide(opts.side), triggerPrice: opts.price, ...sizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:stop-limit")
  .requiredOption("--symbol <symbol>")
  .requiredOption("--side <BUY|SELL>")
  ...
  .action(async (opts) => {
    await resolveForCommand(opts);
    const symbol = normalizeSymbol(opts.symbol);
    printCreated(triggers.add({ kind: "STOP_LIMIT", symbol, side: normalizeSide(opts.side), triggerPrice: opts.stop, limitPrice: opts.limit, ...sizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:take-profit")
  .requiredOption("--symbol <symbol>")
  ...
  .action(async (opts) => {
    await resolveForCommand(opts);
    const symbol = normalizeSymbol(opts.symbol);
    printCreated(triggers.add({ kind: "TAKE_PROFIT", symbol, side: defaultCloseSide(symbol, opts.side), triggerPrice: opts.price, limitPrice: opts.limit, ...exitSizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:stop-loss")
  .requiredOption("--symbol <symbol>")
  .option("--side <BUY|SELL>")
  ...
  .action(async (opts) => {
    await resolveForCommand(opts);
    const symbol = normalizeSymbol(opts.symbol);
    printCreated(triggers.add({ kind: "STOP_LOSS", symbol, side: defaultCloseSide(symbol, opts.side), triggerPrice: opts.price, limitPrice: opts.limit, ...exitSizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:trailing-stop")
  .requiredOption("--symbol <symbol>")
  .option("--side <BUY|SELL>")
  ...
  .action(async (opts) => {
    ...
    printCreated(triggers.add({ kind: "TRAILING_STOP", symbol, side: defaultCloseSide(symbol, opts.side), trailMode: trail.mode, trailValue: trail.value, ...exitSizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:trailing-stop-limit")
  .requiredOption("--symbol <symbol>")
  .option("--side <BUY|SELL>")
  .requiredOption("--trail <amount|percent>", "Trailing distance, e.g. 1000 or 5%")
  ...
  .action(async (opts) => {
    ...
    const symbol = normalizeSymbol(opts.symbol);
    const trail = parseAmountOrPercent(opts.trail);
    printCreated(triggers.add({ kind: "TRAILING_STOP_LIMIT", symbol, side: defaultCloseSide(symbol, opts.side), trailMode: trail.mode, trailValue: trail.value, limitOffset: opts.limitOffset, ...exitSizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:oco")
  .requiredOption("--symbol <symbol>")
  .option("--side <BUY|SELL>")
  ...
  .action(async (opts) => {
    ...
    const children = triggers.addOco([
      { kind: "TAKE_PROFIT", symbol, side, triggerPrice: opts.takeProfit, limitPrice: opts.takeProfitLimit, ...sizing, paymentCurrency: opts.paymentCurrency },
      opts.stopLimit
        ? { kind: "STOP_LIMIT", symbol, side, triggerPrice: opts.stopLoss, limitPrice: opts.stopLimit, ...sizing, paymentCurrency: opts.paymentCurrency }
        : { kind: "STOP_LOSS", symbol, side, triggerPrice: opts.stopLoss, ...sizing, paymentCurrency: opts.paymentCurrency },
    ] as any[], groupId);
    printCreated(children, opts.watch);
  });

commonOrderOptions(program.command("trigger:bracket")
  .requiredOption("--symbol <symbol>")
  ...
  .action(async (opts) => {
    ...
    printCreated(triggers.add({
      ...
      meta: { bracket: { takeProfitPrice: opts.takeProfit, stopLossPrice: opts.stopLoss, stopLimitPrice: opts.stopLimit, useClosePosition: opts.exitsClosePosition } },
    }), opts.watch);
  });

commonOrderOptions(program.command("trigger:scale-out")
  .requiredOption("--symbol <symbol>")
  .option("--side <BUY|SELL>")
  ...
  .action(async (opts) => {
    await resolveForCommand(opts);
    const symbol = normalizeSymbol(opts.symbol);
    printCreated(triggers.add({ kind: "TAKE_PROFIT", symbol, side: defaultCloseSide(symbol, opts.side), triggerPrice: opts.price, limitPrice: opts.limit, closePercentage: opts.percent, reduceOnly: true, paymentCurrency: opts.paymentCurrency, meta: { strategy: "SCALE_OUT" } }), opts.watch);
  });

commonOrderOptions(program.command("trigger:break-even")
  .requiredOption("--symbol <symbol>")
  .option("--side <BUY|SELL>")
  ...
  .action(async (opts) => {
    ...
    printCreated(triggers.add({ kind: "BREAK_EVEN_STOP", symbol, side: defaultCloseSide(symbol, opts.side), activationMode: after.mode, activationValue: after.value, lockMode: plus.mode, lockValue: plus.value, limitPrice: opts.limit, ...exitSizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:close-after")
  .requiredOption("--symbol <symbol>")
  ...
  .action(async (opts) => {
    await resolveForCommand(opts);
    const symbol = normalizeSymbol(opts.symbol);
    printCreated(triggers.add({ kind: "TIME_CLOSE", symbol, side: defaultCloseSide(symbol, opts.side), triggerAt: parseTimeOrDuration(opts.after), limitPrice: opts.limit, ...exitSizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:close-at")
  .requiredOption("--symbol <symbol>")
  ...
  .action(async (opts) => {
    await resolveForCommand(opts);
    const symbol = normalizeSymbol(opts.symbol);
    printCreated(triggers.add({ kind: "TIME_CLOSE", symbol, side: defaultCloseSide(symbol, opts.side), triggerAt: parseTimeOrDuration(opts.at), limitPrice: opts.limit, ...exitSizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

program.command("trigger:cancel-after")
  ...
  .action(async (opts) => {
    ...
    printCreated(triggers.add({ kind: "TIME_CANCEL", symbol, side: "SELL", triggerAt: parseTimeOrDuration(opts.after), cancelTriggerId: opts.id, paymentCurrency: "USD" }), opts.watch);
  });

commonOrderOptions(program.command("trigger:price-band")
  .requiredOption("--symbol <symbol>")
  .requiredOption("--side <BUY|SELL>")
  ...
  .action(async (opts) => {
    ...
    printCreated(triggers.add({ kind: "PRICE_BAND", symbol, side: normalizeSide(opts.side), priceBandMode: mode, upperPrice: opts.upper, lowerPrice: opts.lower, limitPrice: opts.limit, ...sizingFromOptions(opts), paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

commonOrderOptions(program.command("trigger:risk-guard")
  .requiredOption("--symbol <symbol>")
  ...
  .action(async (opts) => {
    ...
    printCreated(triggers.add({ kind: "RISK_GUARD", symbol, side: defaultCloseSide(symbol, opts.side), riskMetric: parseRiskMetric(opts.metric), riskThreshold: opts.threshold, riskAction: action, closePosition: action === "CLOSE_POSITION" || opts.closePosition, limitPrice: opts.limit, quantity: opts.quantity, paymentCurrency: opts.paymentCurrency }), opts.watch);
  });

program.command("trigger:list")
  .option("--all")
  .option("--symbol <symbol>")
  .action((opts) => {
    const status = opts.all ? undefined : "ACTIVE" as any;
    console.log(formatTriggers(triggers.list({ symbol: opts.symbol ? normalizeSymbol(opts.symbol) : undefined, status })));
  });
program.command("trigger:cancel").requiredOption("--id <id>").action((opts) => {
  const t = triggers.cancel(opts.id);
  console.log(t ? `${YELLOW}Cancelled:${RESET}\n${formatTriggers([t])}` : `No trigger found: ${opts.id}`);
});
program.command("trigger:watch").option("--symbol <symbol>").action(async (opts) => { await resolveForCommand(opts); runTriggerWatcher(opts.symbol ? [opts.symbol] : undefined); });

program.command("filledorders:refresh")
...
program.command("llm:connect")
  .requiredOption("--provider <provider>", "openai|anthropic|xai|codex|ovhcloud|gemini|openrouter|groq|huggingface|pollinations|custom-openai")
  ...
  .action((opts) => {
    ...
    const saved = llmConfig.setConnection({ ownerId, provider: opts.provider, model: opts.model, apiKey: opts.apiKey, apiKeyEnv: opts.apiKeyEnv, baseUrl: opts.baseUrl, makeDefault: opts.default, useAsFallback: opts.fallback });
    const keySource = saved.provider === "codex-oauth" ? "Codex OAuth (run codex:connect)" : opts.apiKey ? `stored:${redactedSecret(opts.apiKey)}` : `env:${saved.apiKeyEnv}`;
    printLlmConnectionSaved(saved.provider, saved.model, keySource);
  });

program.command("codex:connect")
  ...
  .action(async (opts) => {
    ...
    const saved = llmConfig.setConnection({ ownerId, provider: "codex-oauth", model: opts.model, makeDefault: opts.default !== false, useAsFallback: false });
...
program.command("codex:status")
  .option("--owner <id>", "LLM/Codex owner namespace", "default")
  .action((opts) => {
    const status = codexOAuthStatus(cliOwner(opts));
    console.log(`${status.connected ? GREEN + "connected" + RESET : YELLOW + "not connected" + RESET} auth=${status.authFile}`);
  });

program.command("codex:logout")
  .option("--owner <id>", "LLM/Codex owner namespace", "default")
  .action((opts) => {
    const removed = logoutCodexOAuth(cliOwner(opts));
    console.log(removed ? `${GREEN}Codex OAuth session removed.${RESET}` : `${YELLOW}No Codex OAuth session found.${RESET}`);
  });

program.command("llm:providers").option("--owner <id>", "LLM/Codex owner namespace", "default").action((opts) => console.log(formatLlmProviderRows(llmConfig.listRows(cliOwner(opts)))));
program.command("llm:fallbacks").action(() => { console.log(`Free/no-subscription fallback order: ${FREE_FALLBACK_ORDER.join(" -> ")}`); console.log("Anonymous providers are used without a key; other free-tier providers are tried when their env key is present. Codex OAuth is opt-in with codex:connect."); });
program.command("llm:plan")
  ...
  .action(async (opts) => {
    const ownerId = cliOwner(opts);
    if (opts.refreshPositions) { await resolveForCommand(opts); await new PositionSyncService(HttpSvc, positions).refresh().catch(() => 0); }
    const planner = new LlmStrategyPlanner(llmConfig);
    const plan = await planner.plan({ ownerId, prompt: opts.prompt, commandFormat: "cli", provider: opts.provider, allowFallback: opts.fallback, defaultPaymentCurrency: process.env.DEFAULT_PAYMENT_CURRENCY ?? "USD", positionsContext: positions.describe(), riskContext: formatRisk(positions), resolveCloseSide: (symbol) => positions.getCloseSide(symbol) as any });
    const draft = llmDrafts.add({ ownerId, prompt: opts.prompt, provider: plan.provider, model: plan.model, format: "cli", summary: plan.summary, commands: plan.commands, riskNotes: plan.riskNotes });
    console.log(formatDraft(draft));
    if (draft.commands.length) console.log(`
Confirm only after review:
  npm run cli -- llm:confirm --id ${draft.id}${ownerId !== "default" ? ` --owner ${ownerId}` : ""}`);
  });
program.command("llm:confirm").requiredOption("--id <draft-id>").option("--owner <id>", "LLM/Codex owner namespace", "default").option("--watch").action(async (opts) => {
  const ownerId = cliOwner(opts);
  await resolveForCommand(opts);
...
program.command("llm:drafts").option("--owner <id>", "LLM/Codex owner namespace", "default").option("--all").action((opts) => { const drafts = llmDrafts.list(cliOwner(opts), opts.all); console.log(drafts.length ? drafts.map(formatDraft).join("\n\n---\n\n") : "No pending LLM drafts."); });
program.command("llm:cancel").requiredOption("--id <draft-id>").option("--owner <id>", "LLM/Codex owner namespace", "default").action((opts) => { const draft = llmDrafts.mark(opts.id, "CANCELLED", cliOwner(opts)); console.log(`Cancelled ${draft.id}.`); });
