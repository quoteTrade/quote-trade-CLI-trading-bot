...
export async function runCodexOAuthLogin(ownerId = "default"): Promise<void> {
  ...
  try {
    chmodSync(codexAuthFile(ownerId), 0o600);
  } catch {
    /* best effort */
  }
  if (!hasCodexOAuthSession(ownerId))
    throw new Error("Codex login finished, but no ChatGPT OAuth session was found in CODEX_HOME/auth.json");
}
...
export async function completeCodexOAuthPlan(
  ...
  if (!hasCodexOAuthSession(owner))
    throw new Error(`Codex OAuth is not connected for owner ${owner}. Run codex:connect first.`);
  ...
  const code = await new Promise<number>((resolve, reject) => {
    ...
    // promise when the child goes silent: a stalled codex never exits and never
    // errors, so neither handler below fires. Unref'ing it let the event loop
    // drain first, which meant the timeout simply never happened. It is cleared
    // on both exit and error, so it cannot hold the process open in normal use.
